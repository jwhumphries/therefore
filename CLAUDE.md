...
```bash
just dev            # Start dev environment (Go:8080 + Vite:3000) via docker-compose
just dev-stop       # Stop dev environment
just check          # Run all checks (lint, lint-frontend, typecheck, test, test-frontend)
just build          # Build production container, load into Docker
```
...
just fmt-frontend   # Format frontend code (ESLint --fix)
just templ-fmt      # Format .templ files
just build-frontend # Build frontend assets only
just ssg            # Generate SSG pages (depends on build-frontend)
just dev-logs       # View dev container logs
just dev-shell      # Shell into dev container
just clean          # Remove build artifacts (tmp, bin, node_modules, dist)
just clean-docker   # Remove Docker volumes and images
```
...
**Templ templates**: After modifying `.templ` files, run `just templ` to regenerate Go code. Generated files are `*_templ.go`.
...
**Go tests**: `just test` runs `go test -v ./...` via Dagger. Test files alongside source (`*_test.go`).
