...
var (
	escapedRegex  = regexp.MustCompile(`\\(\d+)`)
	verseNumRegex = regexp.MustCompile(`(^|\n|\s)(\d{1,3})\s`)
	dropCapRegex  = regexp.MustCompile(`^((?:<sup[^>]*>[^<]*</sup>|\s)*)(\pL)`)
)
...
func formatVerseNumbers(content string) string {
	...
	i := 0
	result := escapedRegex.ReplaceAllStringFunc(content, func(match string) string {
	...
	result = verseNumRegex.ReplaceAllString(result, `$1<sup class="verse-num">$2</sup>`)
...
func applyScriptureDropCap(content string) string {
	...
	return dropCapRegex.ReplaceAllString(content, `$1<span class="scripture-drop-cap">$2</span>`)
}
