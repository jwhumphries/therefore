...
function getAnimationClass(state: AnimationState, type: AnimationType): string {
  if (state === 'exiting') {
    switch (type) {
      case 'page-forward':
        return 'posts-exit-left';
      case 'page-backward':
        return 'posts-exit-right';
      case 'sort':
        return 'posts-exit-down';
    }
  }
  if (state === 'entering') {
    switch (type) {
      case 'page-forward':
        return 'posts-enter-right';
      case 'page-backward':
        return 'posts-enter-left';
      case 'sort':
        return 'posts-enter-up';
    }
  }
  return '';
}

function getAnimationDurations(type: AnimationType): {
  exit: number;
  enter: number;
} {
  if (type === 'sort') {
  ...
  usePageMeta(
    tag
      ? {
          ...
          description: `All posts tagged "${tag}" on Therefore.`,
        }
      : {},
  );
